...
- 支持成对符号输出且上屏字词居于符号内部(现已支持多个平台客户端), 如: `()`, `【】`, `''`等
...
- 方案名称为“飞鹤快拼”，它提供了小鹤双拼+形码辅助选字的输入方案，默认 **开启** 间接辅码。
  ...
  开启后单字三码(音2+形1)、四码(音2+形2)均可直接敲，如 `jiv` / `jivo` → 「籍」。
  代价是三码会成为多音节词的合法前缀：打「输入法」(词库码 `uu ru fa`) 时，
  中途第 5 码 `uuruf` 会被切为 `uur|uf` 而出现「舒/俞/倏」，但打完整的 `uurufa` 结果正确。
  盲打、不看中途候选的用户可视其为显示层瑕疵；在意的用户请保持注释状态，改用引导符补形码。
...
- `/hs` 前缀: 历史上屏记录, 充当简易输入记录器, 聊天记录回溯 😈 , 按下`Ctrl+r` 可将误上屏候选取消记录词频。
...
### 额外的快捷键
  ...
  - 词组的双拼(音码)部分输入完成后, 可用 `` ` `` 键逐字追加辅助码(也支持逐音节选字).
    例如敲 ``lb`m na`sn si`q``, 下次输入`lb na si`可得「林纳斯」
...
- 同文(Trime) 在 [GitHub 页面](https://github.com/osfans/trime) 下载最新版, 本项目已自带适配方案的同文键盘皮肤
...
- [语法模型](https://github.com/boomker/rime-fast-xhup/releases/download/v1.0.0/zh-hans-t-huayu-v7-bgw.gram), 长句输出更懂你的心意(需自行下载)
- [自然快手](https://github.com/functoreality/rime-flypy-zrmfast), 本项目方案最开始受到此项目启发
- [雾凇拼音](https://github.com/iDvel/rime-ice), 本项目方案基础词库由此项目转换而来
- [汉字部件拆字](https://github.com/mirtlecn/rime-radical-pinyin), 本项目汉字反查方案功能来源于它
- [英译中词库](https://github.com/skywind3000/ECDICT), 本项目方案英译中词库由此项目转换而来
